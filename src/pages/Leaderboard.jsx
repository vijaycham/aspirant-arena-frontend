         ...
         <div className="max-w-4xl mx-auto space-y-8">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
               ...
               <div className="p-1 bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 flex items-center shadow-sm">
                  {['daily', 'weekly', 'monthly', 'all-time'].map((r) => (
                     <button
                        key={r}
                        onClick={() => setRange(r)}
                        className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${range === r
...
const PodiumStep = ({ user, rank, getAvatar, getHandle, delay, isCenter }) => {
   return (
      ...
      >
         ...
         {/* Avatar Card */}
         <div className={`relative p-1 rounded-full border-4 shadow-xl mb-4 bg-white dark:bg-slate-900 ${rank === 1 ? 'border-yellow-400 w-20 h-20 md:w-24 md:h-24' :
            ...
            />
            <div className={`absolute -bottom-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-[10px] font-black text-white shadow-lg uppercase tracking-widest ${rank === 1 ? 'bg-yellow-400 text-yellow-900' :
         ...
         {/* Score Card */}
         <div className={`w-full bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-2 md:p-4 text-center shadow-lg relative overflow-hidden group ${isCenter ? 'min-h-[140px] md:min-h-[160px] border-t-4 border-t-yellow-400' :
